# -*- coding: utf-8 -*-
"""重建销售明细导出(含业务员)，验证口径与 sales_agg 一致"""
import json, re
...
# 尝试不同过滤，看总额是否=2,521.01万
for name, cond in [
    ('全部', lambda c: True),
    ('金额!=0', lambda c: num(c.get('J','')) != 0),
    ('已完成', lambda c: c.get('AB','') == '已完成'),
    ('已完成且金额!=0', lambda c: c.get('AB','') == '已完成' and num(c.get('J','')) != 0),
    ('金额!=0且数量!=0', lambda c: num(c.get('J','')) != 0 and num(c.get('I','')) != 0),
]:
    tot = 0.0; lines = 0; orders = set()
    for c in rows[1:]:
        if cond(c):
            amt = num(c.get('J', 0))
            tot += amt; lines += 1; orders.add(c.get('C', ''))
